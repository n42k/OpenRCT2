...
### Correcting it:
The function when called with the correct arguments (which they were doing): `GetLocaleInfo(LOCALE_USER_DEFAULT,	LOCALE_IMEASURE | LOCALE_RETURN_NUMBER,	(PSTR)&fahrenheit, sizeof(country))`, changes the value of `farenheit` to 1 if the current country uses the "United States system" of temperature measurement (Fahrenheit) or to 0 if the country uses the metric system (Celsius). This means that the switch statement can simply be replaced with an if/else statement seeing as we already know what temperature measurement system the current country uses.
